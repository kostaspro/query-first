...
namespace QueryFirst.QueryObj.Helper.Providers
{
    ...
    public class SqlClient : IProvider
    {
        ...
        public virtual List<IQueryParamInfo> FindUndeclaredParameters(string queryText, string connectionString)
        {
                ...
                while (rdr.Read())
                {
                    ...
                    if (rdr.GetString(1).Substring(0, 2) != "@@")
                    {
                        ...
                        myParams.Add(new QueryParamInfo()
                        {
                            ...
                            DbType = !rdr.IsDBNull(3) ? rdr.GetString(3) : rdr.GetString(10),
            ...
            {
                //find user table types
                var cmd=conn.CreateCommand();
                conn.Open();
                cmd.CommandText = "SELECT name FROM sys.types WHERE is_table_type=1";
                var dt = new DataTable();
                new SqlDataAdapter((SqlCommand) cmd).Fill(dt);
                UserTableTypes = dt.Rows.Cast<DataRow>().Select(r => Convert.ToString(r.ItemArray[0])).ToList();
            }

            return myParams;
        }

        public List<string> UserTableTypes { get; set; }    
        ...
        public virtual string TypeMapDB2CS(string DBType, out string DBTypeNormalized, bool nullable = true)
        {
            switch (DBType.ToLower())
            {
                ...
                default:
                    if (true.Equals(UserTableTypes?.Contains(DBType)))
                    {
                        DBTypeNormalized = DBType;
                        return "object";
                    }
